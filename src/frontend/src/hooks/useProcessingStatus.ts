...
export function useProcessingStatus() {
  ...
  const startProcessing = useCallback(
    async (
      ...
      console.log('[useProcessingStatus] Starting processing:', { voiceSampleId, songId });
      ...
      try {
        for (let i = 0; i < stages.length; i++) {
          ...
          console.log(`[useProcessingStatus] Stage ${i + 1}/${stages.length}: ${stage.key}`);
        ...
        console.log('[useProcessingStatus] Creating cover in backend:', { coverId, songId, voiceSampleId });
        
        await actor.createCover(coverId, songId, voiceSampleId, finalMix);
        
        console.log('[useProcessingStatus] Cover created successfully');
        setProgress(100);
        
        // Invoke the callback to navigate to library
        onComplete(coverId, finalMix);
      } catch (err) {
        console.error('[useProcessingStatus] Failed to create cover:', err);
